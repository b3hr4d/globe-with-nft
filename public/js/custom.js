...
const Custom = () => {
    ...
    return (React.createElement(Globe, { ref: globeEl, customLayerData: makeTile, customLayerLabel: ({ id }) => id, customThreeObject: ({ color, phi, theta }) => {
            ...
            tile.lookAt(0, 0, 0);
            return tile;
        }, onCustomLayerClick: ({ lat, lng }, event) => {
            // smothly zoom to tile on click and disable when pinch end
            globeEl.current.pointOfView({ lat, lng, altitude: 0.01 }, 1000);
        } }));
};
ReactDOM.render(React.createElement(Custom, null), document.getElementById("globeViz"));
